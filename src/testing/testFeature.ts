...
import { generatePipeName, inferFriCASNumThreads, registerCommand } from '../utils'
...
export const notifyTypeTextDocumentPublishTests = new NotificationType<PublishTestsParams>('fricas/publishTests')
...
export class TestProcess {
    ...
    public async start(context: vscode.ExtensionContext, fricasExecutablesFeature: FriCASExecutablesFeature, outputChannel: vscode.OutputChannel, projectPath: string, packagePath: string, packageName: string) {
        this.projectPath = projectPath
        this.packagePath = packagePath
        this.packageName = packageName

        const pipename = generatePipeName(v4(), 'vsc-jl-ts')
        ...
        const fricasExecutable = await fricasExecutablesFeature.getActiveFriCASExecutableAsync()
        ...
        if (nthreads!=='auto' && nthreads!=='') {
            jlEnv['FRICAS_NUM_THREADS'] = nthreads
        }
...
export class TestFeature {
    ...
    constructor(private context: vscode.ExtensionContext, private executableFeature: FriCASExecutablesFeature, private workspaceFeature: WorkspaceFeature) {
        try {
            this.outputChannel = vscode.window.createOutputChannel('FriCAS Testserver')
            ...
            context.subscriptions.push(
                registerCommand('language-fricas.stopTestProcess', (node: TestProcessNode) =>
                    node.stop()
                )
            )
            ...
            fileTestitem.children.replace([
                ...params.testitemdetails.map(i => {
                    ...
                    if (params.package_path==='') {
                        testitem.error = 'Unable to identify a FriCAS package for this test item.'
                    }
                    else {
    ...
    async getFreeTestProcess(details: { testitem: TestItemDetail, projectPath: string, packagePath: string, packageName: string }) {
        if(!this.testProcesses.has(JSON.stringify({projectPath: details.projectPath, packagePath: details.packagePath, packageName: details.packageName}))) {
            ...
            let maxNumProcesses = vscode.workspace.getConfiguration('fricas').get<number>('numTestProcesses')
