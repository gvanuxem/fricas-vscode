...
let g_currentFriCASVersion: string = ''
...
export async function init(context: vscode.ExtensionContext) {
    ...
    extensionClient.commonProperties['fricasversion'] = g_currentFriCASVersion
...
export function handleNewCrashReport(name: string, message: string, stacktrace: string, cloudRole: string) {
    if (name.startsWith('LSPrecompileFailure')) {
        vscode.window.showErrorMessage('The FriCAS Language Server failed to precompile. Please check the FAQ and the local output.', 'Open FAQ', 'Open Logs').then(choice => {
            if (choice === 'Open Logs') {
                vscode.commands.executeCommand('language-fricas.showLanguageServerOutput')
            } else if (choice === 'Open FAQ') {
                vscode.commands.executeCommand('vscode.open', vscode.Uri.parse('https://www.fricas-vscode.org/docs/stable/faq'))
            }
        })
    }
...
export function startLsCrashServer() {
    ...
    g_jlcrashreportingpipename = generatePipeName(v4(), 'vsc-jl-cr')
...
async function showCrashReporterUIConsent() {
    if (crashReporterUIVisible || vscode.workspace.getConfiguration('fricas').get<boolean>('enableCrashReporter') === false) {
        return
    }
    else {
        ...
        try {
            ...
            const choice = await vscode.window.showInformationMessage('The FriCAS language extension crashed. Do you want to send more information about the problem to the development team? Read our [privacy statement](https://github.com/gvanuxem/fricas-vscode/wiki/Privacy-Policy) to learn more about how we use crash reports and what data will be transmitted.', agree, agreeAlways, disagree)
            if (choice === disagree) {
                vscode.workspace.getConfiguration('fricas').update('enableCrashReporter', false, true)
            }
            if (choice === agreeAlways) {
                vscode.workspace.getConfiguration('fricas').update('enableCrashReporter', true, true)
            }
...
export function setCurrentFriCASVersion(version: string) {
    g_currentFriCASVersion = version
