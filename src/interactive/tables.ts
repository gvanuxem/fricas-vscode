...
export function displayTable(payload, context, isLazy = false, kernel?: FriCASKernel) {
    const parsedPayload = JSON.parse(payload)
    const title = parsedPayload.name

    const panel = vscode.window.createWebviewPanel('jlgrid', title ? 'FriCAS Table: ' + title : 'FriCAS Table', {
        preserveFocus: true,
    ...
    if (isLazy) {
        ...
        panel.onDidDispose(async () => {
            try {
                ...
                console.debug('Could not dispose of lazy table object on the FriCAS side: ', err)
            }
        })
            ...
            if (message.type === 'getRows') {
                ...
                try {
                    ...
                    if (conn) {
                        ...
                        warning = vscode.window.showWarningMessage('Could not fetch table data. The FriCAS process is no longer available.', button)
                    }
