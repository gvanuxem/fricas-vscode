...
export class Result {
    ...
    constructor(editor: vscode.TextEditor, range: vscode.Range, content: ResultContent) {
        ...
        for (const selection of editor.selections) {
            if (isResultInLineRange(editor, this, selection)) {
                setContext('fricas.hasInlineResult', true)
            }
        }
    }
    ...
    createResultDecoration(): vscode.DecorationRenderOptions {
        const accentColor = this.content.isError
            ? new vscode.ThemeColor('fricas.result.error')
            : new vscode.ThemeColor('fricas.result.success')

        return {
            before: {
                contentIconPath: undefined,
                contentText: undefined,
                color: new vscode.ThemeColor('fricas.result.foreground'),
                backgroundColor: new vscode.ThemeColor('fricas.result.background'),
                margin: '0 0 0 10px',
                border: '2px solid',
...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(
        ...
        // public commands
        registerCommand('language-fricas.clearAllInlineResults', removeAll),
        registerCommand('language-fricas.clearAllInlineResultsInEditor', () => removeAll(vscode.window.activeTextEditor)),
        registerCommand('language-fricas.clearCurrentInlineResult', () => {
        ...
        // internal commands
        registerCommand('language-fricas.openFile', (locationArg: { path: string, line: number }) => {
            openFile(locationArg.path, locationArg.line)
        }),
        registerCommand('language-fricas.gotoFirstFrame', gotoFirstFrame),
        registerCommand('language-fricas.gotoPreviousFrame', (frameArg: { frame: Frame }) => {
            gotoPreviousFrame(frameArg.frame)
        }),
        registerCommand('language-fricas.gotoNextFrame', (frameArg: { frame: Frame }) => {
            gotoNextFrame(frameArg.frame)
        }),
        registerCommand('language-fricas.gotoLastFrame', gotoLastFrame),
        registerCommand('language-fricas.clearStackTrace', clearStackTrace)
    )
    setContext('fricas.supportedLanguageIds', supportedLanguageIds)
}
...
function updateContextKeyForSelections(editor: vscode.TextEditor, selections: readonly vscode.Selection[] = editor.selections) {
    ...
    setContext('fricas.hasInlineResult', false)
}
...
function attachGotoFrameCommandLinks(transformed: string, frame: Frame) {
    return [
        `[\`${GlyphChars.MuchLessThan}\`](${constructCommandString('language-fricas.gotoFirstFrame')} "Goto First Frame")`,
        `[\`${GlyphChars.LessThan}\`](${constructCommandString('language-fricas.gotoPreviousFrame', { frame })} "Goto Previous Frame")`,
        `[\`${GlyphChars.GreaterThan}\`](${constructCommandString('language-fricas.gotoNextFrame', { frame })} "Goto Next Frame")`,
        `[\`${GlyphChars.MuchGreaterThan}\`](${constructCommandString('language-fricas.gotoLastFrame')} "Goto Last Frame")`,
        `[\`${GlyphChars.BallotX}\`](${constructCommandString('language-fricas.clearStackTrace')} "Clear Stack Traces")`,
        `\n${transformed}`
    ].join(' ')
}
