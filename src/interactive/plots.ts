...
const c_fricasPlotPanelActiveContextKey = 'fricas.plotpaneFocus'
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        registerCommand('language-fricas.copy-plot', requestCopyPlot),
        registerCommand('language-fricas.save-plot', requestExportPlot),
        registerCommand('language-fricas.show-plotpane', showPlotPane),
        registerCommand('language-fricas.plotpane-enable', enablePlotPane),
        registerCommand('language-fricas.plotpane-disable', disablePlotPane),
        registerCommand('language-fricas.plotpane-previous', plotPanePrev),
        registerCommand('language-fricas.plotpane-next', plotPaneNext),
        registerCommand('language-fricas.plotpane-first', plotPaneFirst),
        registerCommand('language-fricas.plotpane-last', plotPaneLast),
        registerCommand('language-fricas.plotpane-delete', plotPaneDel),
        registerCommand('language-fricas.plotpane-delete-all', plotPaneDelAll),
        registerCommand('language-fricas.show-plot-navigator', () => g_plotNavigatorProvider.showPlotNavigator()),
        vscode.window.registerWebviewViewProvider('fricas-plot-navigator', g_plotNavigatorProvider)
    )
}
...
class PlotNavigatorProvider implements vscode.WebviewViewProvider {
    ...
    async showPlotNavigator() {
        if (this?.view?.show === undefined) {
            ...
            await vscode.commands.executeCommand('fricas-plot-navigator.focus')
        }
        this.view.show(true)
    }
    ...
    reloadPlotPane() {
        ...
        if (this.plotsInfo.length > 0) {
            ...
            innerHTML = `<p>Use FriCAS to plot and your plots will appear here.</p>`
        }
...
export function showPlotPane() {
    ...
    if (!g_plotPanel) {
        ...
        setContext(c_fricasPlotPanelActiveContextKey, true)

        const configListener = vscode.workspace.onDidChangeConfiguration(config => {
            if (config.affectsConfiguration('fricas') && g_plotPanel) {
        ...
        g_plotPanel.onDidDispose(() => {
            ...
            setContext(c_fricasPlotPanelActiveContextKey, false)
...
function makeTitle() {
    let plotTitle = 'FriCAS Plots'
    if (vscode.workspace.getConfiguration('fricas').get('usePlotPane')) {
...
function enablePlotPane() {
    const conf = vscode.workspace.getConfiguration('fricas')
...
export function displayPlot(params: { kind: string, data: string }, kernel?: FriCASKernel) {
    ...
    else if (kind === 'fricasvscode/html') {
...
async function _writePlotFile(fileName: string, data: FileLike) {
    ...
    const defaultPlotsDir: string = vscode.workspace.getConfiguration('fricas').get('plots.path') ?? ''
    ...
    try {
        ...
        if (!isFile) {
            ...
            if (action === 'Create') {
                ...
                vscode.commands.executeCommand('workbench.action.openSettings', 'fricas.plots.path')
                return
            } else {
