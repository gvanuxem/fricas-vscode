...
abstract class SessionNode extends AbstractWorkspaceNode {
    ...
    constructor() {
        ...
        this._showModules = vscode.workspace.getConfiguration('fricas').get('workspace.showModules')
        vscode.workspace.onDidChangeConfiguration(config => {
            if (config.affectsConfiguration('fricas.workspace.showModules')) {
                this._showModules = vscode.workspace.getConfiguration('fricas').get('workspace.showModules')
...
export class WorkspaceFeature {
    ...
    constructor(private context: vscode.ExtensionContext) {
        ...
        this.context.subscriptions.push(
            ...
            // commands
            registerCommand('language-fricas.showInVSCode', (node: VariableNode) =>
                this.showInVSCode(node)
            ),
            registerCommand('language-fricas.workspaceGoToFile', (node: VariableNode) =>
                this.openLocation(node)
            ),
            registerCommand('language-fricas.showModules', () =>
                this._REPLTreeDataProvider.toggleModules(true)
            ),
            registerCommand('language-fricas.hideModules', () =>
    ...
    public async addNotebookKernel(kernel: FriCASKernel) {
...
implements vscode.TreeDataProvider<AbstractWorkspaceNode>
{
    ...
    getTreeItem(node: AbstractWorkspaceNode): vscode.TreeItem {
        if (node instanceof VariableNode) {
            ...
            const treeItem = new vscode.TreeItem('FriCAS REPL')
            treeItem.description = ''
            treeItem.tooltip = ''
            treeItem.contextValue = 'fricasrepl'
            treeItem.collapsibleState = vscode.TreeItemCollapsibleState.Expanded
            return treeItem
        } else if (node instanceof NotebookNode) {
            const treeItem = new vscode.TreeItem('FriCAS Notebook kernel')
    ...
    toggleModules(show: boolean) {
        ...
        vscode.workspace.getConfiguration('fricas').update('workspace.showModules', show, true)
    }
}
