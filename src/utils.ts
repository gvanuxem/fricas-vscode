...
/**
 * Decides the final value to set the `FRICAS_NUM_THREADS` environment variable to
 * given the `fricas.NumThreads` configuration
 *
 * @remarks
 * The logic is:
 *
 * - if `fricas.NumThreads` has a value, we return that, no matter what.
 *
 * - otherwise, if an env var `FRICAS_NUM_THREADS` exists, we return that.
 *
 * - otherwise, we return an empty string as the value
 *
 * @returns A string to set the value of `FRICAS_NUM_THREADS`
 */
export function inferFriCASNumThreads(): string {
    return ''
    const config: number | string | undefined = vscode.workspace.getConfiguration('fricas').get('NumThreads') ?? undefined
    const env: string | undefined = process.env['FRICAS_NUM_THREADS']
